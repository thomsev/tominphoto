  ...
  return <ContactPage>
    ...
    <ContactContent id="kontakt">
      <motion.div initial={reduced ? false : { opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .7, ease: 'easeOut' }}>
        <span className="eyebrow">PRINT &amp; HENVENDELSER</span>
        {thanks ? <div className="thank-you"><span className="success-icon"><Icon name="check" size={32}/></span><h1>Takk for<br/><em>henvendelsen.</em></h1><p>Henvendelsen er sendt. Vi tar kontakt på e-post for å svare på spørsmålene dine.</p><p className="small">Dette er en uforpliktende henvendelse. Eventuelle kjøp avtales sammen.</p><a className="return-link" href="/#arbeider">Tilbake til bildene <Icon /></a></div> : <>
          <h1>Fant du et bilde<br/><em>du vil ta med hjem?</em></h1>
          <p className="lead">Ønsker du et print, eller har du spørsmål om et fotografi? Fortell hvilket bilde du er interessert i, så finner vi riktig størrelse og utførelse sammen.</p>
          <BookingForm name="booking" method="POST" action="/takk/" data-netlify="true" data-netlify-honeypot="bot-field" onSubmit={onSubmit} aria-busy={sending}>
            ...
            <fieldset className="type-picker"><legend>01 / Hva gjelder henvendelsen?</legend><div className="choices">{['Print', 'Bruk av bilder', 'Annet'].map((type,i)=><label key={type}><input type="radio" name="session-type" value={type} required defaultChecked={i===0}/><span>{type}</span></label>)}</div></fieldset>
            ...
            <fieldset><legend>03 / Din henvendelse</legend><div className="fields single">
              <input type="hidden" name="date" value="" /><label>Hvilket fotografi? <small>(valgfritt)</small><input name="photograph" placeholder="Bildets navn, nummer eller en kort beskrivelse" maxLength={240}/></label>
              <label>Din melding <span>*</span><textarea name="message" placeholder="Fortell hva du er interessert i, eller hva du lurer på." rows={4} required minLength={10} maxLength={5000}/></label>
            </div></fieldset>
            <p className="form-note">Du trenger ikke ha bestemt deg for størrelse eller format. Vi finner ut av det sammen. Felter merket * må fylles ut.</p>
            <button type="submit" disabled={sending}>{sending ? 'Sender henvendelse …' : 'Send henvendelse'}<Icon name="diagonal" size={24}/></button>
